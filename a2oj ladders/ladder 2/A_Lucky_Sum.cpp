...
void util(vector<ll>& luckyNumber)
{
    ll i=0;
    luckyNumber.push_back(0);
    while(luckyNumber.back()<1e9){
        luckyNumber.push_back(luckyNumber[i]*10+4);
        luckyNumber.push_back(luckyNumber[i++]*10+7);
    }
}
...
void solve()
{
    ...
    while(l<=r)
    {
        if(luckyNumber[i]>=l)
        {
            ans+=(min(r, luckyNumber[i])-l + 1)*luckyNumber[i];
            l = min(r, luckyNumber[i]) + 1;
        }
        else i++;
    }
    cout<<ans;
